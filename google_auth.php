<?php
// Include the Google Authenticator class definition
...
// Define class for authentication with direct Google Authenticator implementation
class MalayaSolarAuth {
    private $authenticator;
    private $lib_available = false;
    
    public function __construct() {
        // Initialize the Google Authenticator class directly
     ...
     * Uses the Google Authenticator library's QR code generation
     */
    public function getQRCodeUrl($name, $secret, $issuer = 'Malaya Solar') {
        if ($this->lib_available) {
            return $this->authenticator->getQRCodeGoogleUrl($name, $secret, $issuer);
        }
        
        // Fallback to directly using QR Server API if the class isn't available
        $otpauthUrl = 'otpauth://totp/' . rawurlencode($issuer . ':' . $name) . 
                     '?secret=' . $secret . '&issuer=' . rawurlencode($issuer);
        
        return "https://api.qrserver.com/v1/create-qr-code/?data=" . urlencode($otpauthUrl) . "&size=200x200&ecc=M";
    }
